...
- (ArticleCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSLog(@"the key is: %@", cell.theKey);
    NSLog(@"article key is %@", currentArticle.key);
    ...
    }else if (currentArticle.imageURL != nil){
    ...
    {
        UIImage *defaultImage = [UIImage imageNamed:@"virginiatech-logo.jpg"];
       [self.ImagesCacheDictionary setObject:defaultImage forKey:key];
